...
export class LotteryKit {
	...
	public infiniteApprove = async (from: string) => {
		...
		const allowance = await goldToken.allowance(from, this.contractAddress);
		if (allowance.gt(1e34)) {
			return null;
		} else {
			return goldToken.approve(
				this.contractAddress,
				new BigNumber(1e35).toFixed(0)
			);
		}
	};
	...
	public withdrawStart = async (savingsAmount: BigNumber.Value) => {
		...
		const nonvoting = await lockedGold.getAccountNonvotingLockedGold(
			this.contractAddress
		);
		const toRevoke = BigNumber.maximum(nonvoting.negated().plus(toUnlock), 0);

		const votedGroups = await election.getGroupsVotedForByAccount(
			this.contractAddress
		);
		let pendingRevoke = {
		...
		if (votedGroups.length > 0) {
			...
			const votes = await election.getVotesForGroupByAccount(
				this.contractAddress,
				revokeGroup
			);
			...
			pendingRevoke = await election.findLesserAndGreaterAfterVote(
				votes.group,
				toRevokePending.negated()
			);
			activeRevoke = await election.findLesserAndGreaterAfterVote(
				votes.group,
				toRevoke.negated()
			);
		}
	...
	public pendingWithdrawals = async (
		account: Address
	): Promise<PendingWithdrawal[]> => {
		const pendings = await this.contract.methods
			.pendingWithdrawals(account)
			.call();
		return pendings[0].map(
			(v, idx) =>
				({
	...
	public withdrawIndexGlobal = async (
		pendings: PendingWithdrawal[],
		index: number
	) => {
		const lockedGold = await this.kit.contracts.getLockedGold();
		const pendingsGlobal = await lockedGold.getPendingWithdrawals(
			this.contractAddress
		);
		const idx = pendingsGlobal.findIndex(
			(p) =>
				p.value.eq(pendings[index].value) && p.time.eq(pendings[index].time)
		);
		if (idx === -1) {
			throw Error(
				`{time: ${pendings[index].time}, value: ${pendings[index].value}} not found in pending withdrawals`
			);
		}
		return idx;
	};
