...
async function main() {
	...
	const contractSavingsCELOVoterV1 = await readAddressOrDeployContract(
		...
		SavingsCELOVoterV1Json.bytecode +
			kit.web3.eth.abi
				.encodeParameters(["address"], [contractSavingsCELO])
				.slice(2)
	);
	...
	const savingsKit = new LotteryKit(kit, contractSavingsCELO);
	const voterAddr = await savingsKit.contract.methods._voter().call();
	if (voterAddr !== contractSavingsCELOVoterV1) {
		console.info(
			`SavingsCELO authorizing voter: ${contractSavingsCELOVoterV1}...`
		);
		const txo = toTransactionObject(
			kit.connection,
			savingsKit.contract.methods.authorizeVoterProxy(
				contractSavingsCELOVoterV1
			)
		);
